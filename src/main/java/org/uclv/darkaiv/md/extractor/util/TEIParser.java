 ...
 * the License.
 */
package org.uclv.darkaiv.md.extractor.util;

import org.uclv.darkaiv.reviewer.util.*;
...
public class TEIParser {
    ...
    private void parseBiblStruct(JSONObject biblStruct, HashMap<String, Object> metadata) {
        ...
        if (biblStruct.has("monogr")
            ...
            if (monogr.has("title") && (monogr.get("title") instanceof JSONObject)) {
               
                JSONObject title = monogr.getJSONObject("title");
                metadata.put("source", title.getString("content"));
                //System.out.println("<meeting>");
                //System.out.println(monogr.get("meeting"));
            }
